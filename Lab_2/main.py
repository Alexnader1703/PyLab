...
while hero1.is_alive() and hero2.is_alive():
    # Запускаем цикл, который будет продолжаться, пока оба воина живы.
...
if hero1.is_alive():
    print(f"{hero1.name} победил!")  # Если первый воин жив, объявляем его победителем.
else:
    print(f"{hero2.name} победил!")  # Иначе объявляем второго воина победителем.